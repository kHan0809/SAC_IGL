...
import argparse
import os
from model.SAC_HER_OVER import SAC_HER_OVER
...
if __name__ == "__main__":
    ...
    # Experiment
    parser.add_argument("--policy", default="SAC+HER+OVER")  # Policy name
    ...
    # parser.add_argument("--update_start_steps", default=1000, type=int)  # How often (time steps) we evaluate
    ...
    eval_env.seed(args.seed + 100)    # Set seeds
    ...
    # Initialize policy
    agent = SAC_HER_OVER(env_params, env, args)
    obs_np, act_np = agent.pick2np(args.env)
    agent.dataset_all(obs_np, act_np)
