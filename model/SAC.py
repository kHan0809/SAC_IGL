...
class SAC(object):
    def __init__(self, state_dim, action_dim, args):
        ...
        self.critic           = Double_Q(state_dim, action_dim, args.hidden_dim).to(self.device)
    ...
    def update_parameters(self, batch_size=256):
        state_batch, action_batch, next_state_batch, reward_batch, done_batch = self.buffer.sample(batch_size=batch_size)

        self.critic_optimizer.zero_grad()
        critic_loss, Q1, Q2 = self.compute_loss_q(state_batch, action_batch, next_state_batch, reward_batch, done_batch)
        # Optimize the critic
        ...
        if self.automatic_entropy_tuning:
            ...
            alpha_loss = self.compute_loss_alpha(state_batch, action_batch, next_state_batch, reward_batch, done_batch)
            alpha_loss.backward()
    ...
    def compute_loss_q(self,state_batch, action_batch, next_state_batch, reward_batch, done_batch):
        ...
        with torch.no_grad():
            ...
            target_y = reward_batch + self.gamma*(1.-done_batch)*(minq-self.alpha*next_log_pi.reshape(-1,1))

        critic_loss = F.mse_loss(Q1,target_y) + F.mse_loss(Q2,target_y)

        return critic_loss, Q1, Q2
    def compute_loss_pi(self,state_batch, action_batch, next_state_batch, reward_batch, done_batch):
    ...
    def compute_loss_alpha(self,state_batch, action_batch, next_state_batch, reward_batch, done_batch):
        pi, log_pi = self.policy(state_batch)
