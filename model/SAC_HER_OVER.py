...
class SAC_HER_OVER(object):
    def __init__(self, env_params, env, args):
        ...
        self.bc_batch_size = args.batch_size//8

        self.lamb1 = 1e-3
        self.lamb2 = 1.0/self.bc_batch_size
    ...
    def compute_loss_pi(self,state_batch, action_batch, next_state_batch, reward_batch):
        ...
        q1_poly, q2_poly = self.critic(demo_state, pi_demo)
        min_q_poly = torch.minimum(q1_poly, q2_poly)

        q1_demo, q2_demo = self.critic(demo_state, demo_action)
        min_q_demo = torch.minimum(q1_demo, q2_demo)
        q_filter=torch.where(min_q_demo>min_q_poly,1,0)

        bc_loss = ((demo_action-pi_demo) ** 2 * q_filter).mean()

        return self.lamb1 * policy_loss + self.lamb2 * bc_loss
